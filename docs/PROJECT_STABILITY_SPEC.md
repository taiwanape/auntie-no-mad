...
- 手動 `workflow_dispatch` 未指定 `publish=true` 時，只做現有站台健康檢查，不可產生新內容或 AI 圖片。
- `npm run ops:health` 檢查日更 freshness 時必須看最新 `schedule` 事件，不可讓手動 dry-run 掩蓋自動排程失敗。
...
- 歷史重複主圖需透過 `Regenerate Duplicate Image Debt` 手動 workflow 重生；2026-06-04 清理後，`pagesToRegenerate` 必須維持 0。
