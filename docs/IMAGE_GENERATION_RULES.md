...
所有公開主圖都必須先套用 `IMAGE_STYLE_RULE_VERSION` 目前版本的共用規則。現在版本是 `auntie-reference-v8`：
...
這套規則可以大幅降低跑掉機率，也能防止腳本繞過規則。`auntie-reference-v8` 已比純文字 prompt、v6、v7 更硬，因為生成時會帶參考圖、要求不可簡化參考圖、禁止複製參考圖手機姿勢、要求完整故事場景，並使用較高品質輸出；但模型仍不是逐像素模板，仍不能 100% 保證每張都完全貼近參考圖。
