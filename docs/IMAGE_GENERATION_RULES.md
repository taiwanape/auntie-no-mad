...
這份文件是給人看的規格；真正被程式引用的單一來源是 `scripts/image-style-rules.mjs`。每日圖片、歷史舊圖補圖、重複主圖重生，都必須使用同一份 style lock。

## 為什麼以前會跑掉

圖片模型不是照稿排版工具。即使 prompt 寫得很長，它仍會用機率方式補細節，所以只靠「請畫成阿姨風格」不可能保證每天都穩。
...
只要其中一份 prompt 比較鬆、少寫人物限制、少寫禁止文字，生成結果就會開始偏掉。

## 現在的硬規則

所有公開主圖都必須先套用 `IMAGE_STYLE_RULE_VERSION` 目前版本的共用規則。現在版本是 `auntie-style-v5`：
...
這套規則可以大幅降低跑掉機率，也能防止腳本繞過規則。但只靠文字 prompt 仍不能 100% 保證模型每張都完全貼近參考圖。

若之後還要更硬，可以再加第二層：

- 生成後用 vision 模型做風格審查，不合格就重生。
- 使用固定角色參考圖做 image-to-image / edit 流程。
- 每日只發布通過人工或自動視覺審查的圖片。
